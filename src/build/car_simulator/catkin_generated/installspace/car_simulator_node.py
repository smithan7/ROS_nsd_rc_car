...
import rospy
from esp32_interface.msg import (PWM_Cmd, PWM_Measure)
...
class SimulatedCarState:
    ...
    def read_INS(self, event=None):
        ...
        self.input_steering(self.sent_steer)
        self.input_accel(self.sent_speed)
        temp_time = time.time()
    ...
    def cmds_sub_callback(self, msg):
        ...
        self.received_speed = msg.speed
        self.received_steer = msg.steer
    ...
    def write_serial_comands(self, event=None):
        ...
        if self.received_input:
            self.sent_steer = self.received_steer
            self.sent_speed = self.received_speed
            self.received_input = False
