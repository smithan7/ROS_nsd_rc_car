...
class Track:
    ...
    def find_nearest_edge(self, p):
        dists = np.array(self.track_df.apply(lambda x: self.nearest_point_on_line_segment([x["x"], x["y"]], [x["xn"], x["yn"]], p), axis=1).tolist())
        # print("dists: ", dists)
    ...
    def find_nearest_edge_with_seed(self, p, i, window_size=5):
        ...
        dists = np.array(self.track_df[i:cnt].apply(lambda x: self.nearest_point_on_line_segment([x["x"], x["y"]], [x["xn"], x["yn"]], p), axis=1).tolist())
        # print(dists)
        mindex = np.argmin(dists[:,2])
    ...
    def computeRadiusOfCurvature(self, si):
        ...
        if (z1 == z2) or (z2 == z3) or (z3 == z1):
            raise ValueError(str("Duplicate points: " + str(z1) + str(z2) + str(z3)))
    ...
    def get_speed_limits_from_roc(self, car_max_speed, car_max_centripetal_accel):
        rocs = []
        for i in range(len(self.track_df)):
            rocs.append(self.computeRadiusOfCurvature(i))

        self.track_df['roc'] = rocs
        self.track_df['speed limit'] = self.track_df.apply(lambda x: min(car_max_speed, math.sqrt(x['roc'] * car_max_centripetal_accel)), axis=1)
    
    def back_propagate_speed_limits_to_set_speed(self, car_max_braking_accel):
        # start at edge 0 and go to end - 1
        spds = self.track_df['speed limit'].tolist()
        flag = True
        cntr = 0
        while flag and cntr < 100:
            ...
            for i in range(len(spds) - 1):
                ...
                vf = math.sqrt(vi**2 - 2.0*car_max_braking_accel*d)
                if vf < spds[i]:
                    spds[i] = vf
                    flag = True
